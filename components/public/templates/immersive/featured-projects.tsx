...
export default function ImmersiveFeaturedProjects({ projects }: FeaturedProjectsProps) {
  ...
  return (
    <section ref={containerRef} className="relative bg-[var(--bg-primary)] text-[var(--text-primary)] border-t border-[var(--border-subtle)]">
      {/* Title Section (Sticky on desktop) */}
      <div className="lg:sticky lg:top-0 lg:h-screen lg:w-1/3 lg:float-left flex flex-col justify-center px-4 sm:px-12 md:px-24 py-24 lg:py-0 z-10 bg-[var(--bg-primary)]/90 backdrop-blur-xl lg:bg-transparent lg:backdrop-blur-none">
        <motion.div
        ...
        >
          ...
          <p className="text-[var(--text-secondary)] text-lg sm:text-xl max-w-sm leading-relaxed">
            A curated collection of projects highlighting my approach to complex engineering and design problems.
          </p>
        </motion.div>
      </div>
...
function ProjectCard({ project, index, mainImage, prefersReducedMotion }: any) {
  ...
  return (
    ...
    >
      {/* Image with Parallax */}
      <div className="relative aspect-[4/3] sm:aspect-[16/9] w-full rounded-3xl overflow-hidden mb-8 bg-[var(--bg-surface)] border border-[var(--border-subtle)] shadow-xl">
        ...
        <div className="absolute top-6 left-6 z-20 overflow-hidden pointer-events-none">
          <motion.div 
            ...
            className="text-6xl font-black text-white/70 drop-shadow-md"
          >
        ...
        <div className="flex-grow max-w-xl">
          ...
          <div className="flex flex-wrap gap-2">
            {project.techStack?.map((tech: string, i: number) => (
              <span key={i} className="px-3 py-1 bg-[var(--bg-surface)] text-[var(--text-secondary)] rounded-full text-xs sm:text-sm font-medium border border-[var(--border-subtle)]">
                {tech}
              </span>
            ))}
          </div>
